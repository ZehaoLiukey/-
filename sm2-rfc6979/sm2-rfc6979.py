...
import math
import random
fron Crypto.Signature import DSS
...
def KDF(Z, klen):
    ...
    for i in range(1, math.ceil(klen / v) + 1):
        Ha[i] = func.SM3_digest(Z + bin(ct)[2:].zfill(32))
        ct += 1
    # klen/v is integer
...
def SM2_encrypt(M, n, Gx, Gy, a, b, p, Px, Py):
    ...
    C2 = Xor(M, t)
    # A7
    C3 = func.SM3_digest(x2_bit + M + y2_bit)
    # A8
    C = [C1, C2, C3]
    return C
...
def SM2_decrypt(C, n, Gx, Gy, a, b, p, d):
    ...
    MM = Xor(C2, t)
    # B6
    u = func.SM3_digest(x2_bit + MM + y2_bit)
    if (u != C3):
        return False
    # B7
    return MM
...
if __name__ == "__main__":
    ...
    print('d:', hex(d), '\nPx:', hex(Px), '\nPy:', hex(Py))
    msg = 'sducstlzh'
    signer = DSS.new(d, 'deterministic-rfc6979')
    hasher = SM2_encrypt.new(msg.encode())
